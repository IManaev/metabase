...
(defn- canonicalize-filter [[filter-name & args, :as filter-subclause]]
  (cond
    ...
    (recur
     (cons filter-name (mapcat (fn [subclause]
    ...
    :else
    (throw (IllegalArgumentException. (str (tru "Illegal filter clause: {0}" filter-subclause))))))
...
(defn- canonicalize-top-level-mbql-clauses
  ...
  [mbql-query]
  (cond-> mbql-query
    (seq (:aggregation mbql-query)) (update :aggregation canonicalize-aggregations)
    (seq (:breakout    mbql-query)) (update :breakout    (partial mapv wrap-implicit-field-id))
    (seq (:fields      mbql-query)) (update :fields      (partial mapv wrap-implicit-field-id))
    (seq (:filter      mbql-query)) (update :filter      canonicalize-filter)
    (seq (:order-by    mbql-query)) (update :order-by    canonicalize-order-by)))
...
(defn normalize-inner-mbql-query
  "Normalize just the inner part of an *MBQL* query, useful for things like `Metric` definitions."
  [inner-query]
  (:query (normalize {:query inner-query})))
